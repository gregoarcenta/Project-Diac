...
export class AgregarDocenteComponent implements OnInit {
  ...
  validCampo(campo: string) {
    if (
      campo === "contraseña" &&
      this.docenteForm.controls["contraseña"].errors?.["minlength"]
    ) {
      return {
        messagge: `La ${campo} debe tener al menos 6 caracteres`,
        valid: false,
      };
    }
    if (
      this.docenteForm.controls[campo].errors &&
      this.docenteForm.controls[campo].touched
    ) {
  ...
  validSelect(campo: string) {
    if (
      (!this.docenteForm.controls[campo].value ||
        this.docenteForm.controls[campo].value === "0") &&
      this.docenteForm.controls[campo].touched
    ) {
      return { messagge: `El campo ${campo} es obligatorio`, valid: false };
    } else {
  ...
  guardarDocenteConfirm() {
    ...
    if (
      !this.validSelect("rol").valid ||
      !this.validSelect("asignatura").valid
    ) {
      return;
    }
